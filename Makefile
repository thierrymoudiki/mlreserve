...
lint: ## check style with flake8
	flake8 mlreserve tests

coverage: ## check code coverage quickly with the default Python	
	coverage report --omit="venv/*,mlreserve/tests/*" --show-missing

docs: install ## generate docs		
	pip install black pdoc 
	black mlreserve/* --line-length=80	
	find mlreserve/ -name "*.py" -exec autopep8 --max-line-length=80 --in-place {} +
	pdoc -t docs mlreserve/* --output-dir mlreserve-docs
	find . -name '__pycache__' -exec rm -fr {} +
...
build-site: docs ## export mkdocs website to a folder		
	cp -rf mlreserve-docs/* ../../Pro_Website/Techtonique.github.io/mlreserve
	find . -name '__pycache__' -exec rm -fr {} +
...
run-tests: install ## run all the tests with one command
	...
	python3 -m coverage run -m unittest discover -s mlreserve/tests -p "*.py"	
